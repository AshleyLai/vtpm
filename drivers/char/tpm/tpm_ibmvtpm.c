...
static int ibmvtpm_crq_get_rtce_size(struct ibmvtpm_dev *ibmvtpm)
{
	...
	((struct ibmvtpm_crq *)buf)->valid = (u8)IBMVTPM_VALID_CMD;
	((struct ibmvtpm_crq *)buf)->msg = (u8)VTPM_GET_RTCE_BUFFER_SIZE;
...
static int ibmvtpm_crq_get_version(struct ibmvtpm_dev *ibmvtpm)
{
	...
	((struct ibmvtpm_crq *)buf)->valid = (u8)IBMVTPM_VALID_CMD;
	((struct ibmvtpm_crq *)buf)->msg = (u8)VTPM_GET_VERSION;
...
static int ibmvtpm_crq_send_init_complete(struct ibmvtpm_dev *ibmvtpm)
{
	...
	rc = ibmvtpm_send_crq(ibmvtpm->vdev, 0xC002000000000000LL, 0);
	if (rc != H_SUCCESS)
		dev_err(ibmvtpm->dev,
...
static int ibmvtpm_crq_send_init(struct ibmvtpm_dev *ibmvtpm)
{
	...
	rc = ibmvtpm_send_crq(ibmvtpm->vdev, 0xC001000000000000LL, 0);
	if (rc != H_SUCCESS)
		dev_err(ibmvtpm->dev,
...
static int __devexit tpm_ibmvtpm_remove(struct vio_dev *vdev)
{
	...
	dma_unmap_single(ibmvtpm->dev, ibmvtpm->crq_dma_handle, PAGE_SIZE,
			 DMA_BIDIRECTIONAL);
...
{
	struct ibmvtpm_dev *ibmvtpm = ibmvtpm_get_data(&vdev->dev);
	return (PAGE_SIZE + ibmvtpm->rtce_size);
}
...
static int tpm_ibmvtpm_suspend(struct device *dev)
{
	...
	((struct ibmvtpm_crq *)buf)->valid = (u8)IBMVTPM_VALID_CMD;
	((struct ibmvtpm_crq *)buf)->msg = (u8)VTPM_PREPARE_TO_SUSPEND;
...
static int ibmvtpm_reset_crq(struct ibmvtpm_dev *ibmvtpm)
{
	...
	memset(ibmvtpm->crq_queue.crq_addr, 0, PAGE_SIZE);
	ibmvtpm->crq_queue.index = 0;

	rc = plpar_hcall_norets(H_REG_CRQ, ibmvtpm->vdev->unit_address,
				ibmvtpm->crq_dma_handle, PAGE_SIZE);
	if (rc != H_SUCCESS)
		dev_err(ibmvtpm->dev,
			"ibm_reset_crq failed to register rc=%d\n", rc);
	return rc;
}
...
static int tpm_ibmvtpm_resume(struct device *dev)
{
	...
	if ((rc = ibmvtpm_crq_send_init(ibmvtpm)))
...
static struct dev_pm_ops tpm_ibmvtpm_pm_ops = {
 ...
 *	Nothing
 */
static void ibmvtpm_crq_process(struct ibmvtpm_crq *crq, struct ibmvtpm_dev *ibmvtpm)
{
	int rc = 0;
	...
	case VALID_INIT_CRQ:
		...
		case INIT_CRQ:
			...
			if ((rc = ibmvtpm_crq_send_init_complete(ibmvtpm)))
				dev_err(ibmvtpm->dev, "Unable to send CRQ init complete rc=%d\n", rc);
			return;
		case INIT_CRQ_COMP:
			dev_info(ibmvtpm->dev, "CRQ initialization completed\n");
			return;
		default:
				   ...
				   const struct vio_device_id *id)
{
	...
	if (!(np = of_find_node_by_name(NULL, "ibm,vtpm"))) {
		dev_err(dev, "IBMVTPM not supported\n");
		return -ENODEV;
	}

	if (!(chip = tpm_register_hardware(dev, &tpm_ibmvtpm))) {
	...
	if (!crq_q->crq_addr) {
		...
		goto ibmvtpm_cleanup;
	}

	crq_q->num_entry = PAGE_SIZE / sizeof(*crq_q->crq_addr);
	...
	if ((rc = request_irq(vio_dev->irq, ibmvtpm_interrupt, 0,
			      tpm_ibmvtpm_driver_name, ibmvtpm))) {
		dev_err(dev, "Error %d register irq 0x%x\n", rc, vio_dev->irq);
		goto init_irq_cleanup;
	}

	if ((rc = vio_enable_interrupts(vio_dev))) {
	...
	if ((rc = ibmvtpm_crq_get_version(ibmvtpm)))
		goto init_irq_cleanup;

	if ((rc = ibmvtpm_crq_get_rtce_size(ibmvtpm)))
		goto init_irq_cleanup;
...
init_irq_cleanup:
	tasklet_kill(&ibmvtpm->tasklet);
	do {
		rc = plpar_hcall_norets(H_FREE_CRQ, vio_dev->unit_address);
	} while (rc == H_BUSY || H_IS_LONG_BUSY(rc));
reg_crq_cleanup:
	dma_unmap_single(dev, ibmvtpm->crq_dma_handle, PAGE_SIZE,
			 DMA_BIDIRECTIONAL);
map_cleanup:
